...
class InventoryRepositoryImpl(
    ...
    override suspend fun getById(id: Uuid): InventoryItem? = withContext(Dispatchers.IO) {
        inventoryItemDao.getByIdWithImages(id)?.toInventoryItem()
    }

    override suspend fun upsert(inventoryItem: InventoryItem) = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (profile.dataSynchronization == DataSynchronization.UPLOAD_AND_DOWNLOAD ||
                ...
                if (dirtyEntities.isNotEmpty()) {
                    ...
                    try {
                        val syncedItems = inventoryNetworkService.pushInventoryItems(
